...
Class Master extends DBConnection {
		...
		if($this->capture_err())
			...
			if($save){
				if ($this->settings->userdata('id')) {
					...
					$resp['cart_count'] = $this->conn->query("SELECT SUM(quantity) as items from `temporary_cart` where session ='".$_SESSION['visit']."'")->fetch_assoc()['items'];
				}
				$resp['status'] = 'success';
			}else{
				$resp['status'] = 'failed';
	...
	function update_cart_qty(){
		...
		if ($this->settings->userdata('id')) {
			...
			$save = $this->conn->query("UPDATE `temporary_cart` set quantity = '{$quantity}' where id = '{$id}'");
		}
		if($this->capture_err())
			return $this->capture_err();
		if($save){
	...
	function empty_cart(){
		...
		if ($this->settings->userdata('id')) {
			...
			$delete = $this->conn->query("DELETE FROM `temporary_cart` where session = '".$_SESSION['visit']."'");
		}
		if($this->capture_err())
			return $this->capture_err();
		if($delete){
	...
	function delete_cart(){
		...
		if ($this->settings->userdata('id')) {
			...
			$delete = $this->conn->query("DELETE FROM `temporary_cart` where id = '{$id}'");
		}
		if($this->capture_err())
			return $this->capture_err();
		if($delete){
